...
public class PhoneSMS {
	...
	public static int sendSMS(String  messageContent, String phone){
		...
		params.add("username", "czj888hy");
		params.add("password", MD5Util.MD5Encode(MD5Util.MD5Encode("Hsmb4r", "utf-8") + dateTime, "utf-8"));
	...
	public static String messageModel(String mobile_code,int modelType){
		...
		if(modelType==0){
			sbf.append("【深圳维码物联共享陪护床】恭喜您,您在本平台注册的用户账号已经通过了审核,可以登陆app进行使用!");
		}
		if(modelType==1){
			sbf.append("【深圳维码物联共享陪护床】验证码："+mobile_code+".有效期为3分钟,当前操作为完善用户信息!");
		}
		if(modelType==2){
			sbf.append("【深圳维码物联共享陪护床】验证码："+mobile_code+"。有效期为3分钟，当前操作为修改手机!");
		}
		if(modelType==3){
			sbf.append("【深圳维码物联共享陪护床】验证码："+mobile_code+"。有效期为3分钟，当前操为验证新手机!");
		}
		if(modelType==4){
			sbf.append("【深圳维码物联共享陪护床】验证码："+mobile_code+"。有效期为3分钟，当前操作为认购商!");
		}
		if(modelType==5){
			sbf.append("【深圳维码物联共享陪护床】验证码："+mobile_code+"。有效期为3分钟,当前操作为绑定微信!");
		}
		if(modelType==6){
			sbf.append("【深圳维码物联共享陪护床】验证码："+mobile_code+"。有效期为3分钟，当前操作为注册!");
		}
		if(modelType==7){
			sbf.append("【深圳维码物联共享陪护床】验证码："+mobile_code+"。有效期为3分钟，当前操作为登录!");
		}
		return sbf.toString();
	}
...
//		int num=sendSMS(messageModel(str,1),"18814139173");
		int num=sendSMS("【深圳维码物联】温馨提示:您预约的车位还有15分钟到期，请及时离场!","15019200770");
